...
class SubscriptionManager {
  ...
  bindEvents() {
    ...
    if (addQuickBtn) addQuickBtn.addEventListener('click', () => this.openModal());
  ...
  renderSubscriptions() {
    ...
    if (this.subscriptions.length === 0) {
      container.innerHTML = '<p style="text-align: center; opacity: 0.7;">暂无订阅，点击添加按钮开始管理</p>';
      return;
    }
  ...
  updateStats() {
    ...
    const monthly = this.subscriptions.filter(sub => sub.cycle === 'month').reduce((sum, sub) => sum + sub.price, 0);
    const yearly = this.subscriptions.filter(sub => sub.cycle === 'year').reduce((sum, sub) => sum + sub.price, 0);
